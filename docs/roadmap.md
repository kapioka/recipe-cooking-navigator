...
2026-09-05: 検索・タグ機能はローカル作業ツリーに実装され、自動テスト19件・解析・debug buildが成功。Pixelへ更新し既存Recipeファイル保持、ホーム表示、検索結果なし、詳細遷移まで確認した。ユーザー指示で実機確認を中断しており、タグ編集・再起動後の永続化は実機未確認。下記の検索・タグの完了チェックはローカル実装状況を示し、今回の設計書コミットにアプリコードは含めない。

電子書籍型Cooking modeは[cooking-mode-ui.md](cooking-mode-ui.md)へ設計として記録した。UI・進行状態保存の実装は未着手。
...
- [ ] 読むモードから「調理開始」で作るモードへ切り替え
- [ ] 左右30%タップ、左右スワイプで前後工程へ移動
- [ ] 中央40%タップで操作UIを表示・非表示
- [ ] 操作UI非表示でも必要な工程情報・安全情報・音声状態・タイマーを維持
- [ ] スクロール・ボタン・パネル操作との競合および二重遷移を防止
- [ ] 現在工程 / 総工程数を常時表示
- [ ] 工程一覧から任意工程へ移動（移動と作業完了を分離）
- [ ] Recipe ID・revision・Step IDで途中位置を自動保存
- [ ] 「続きから」「最初から」、再開参照欠損時のエラー表示
- [ ] 先頭・末尾・長文・TalkBackの操作を検証
